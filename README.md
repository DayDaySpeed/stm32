...
- [中文文档](#中文文档)
  ...
  - [2.5 教学型工业级模板收敛（v5）](#25-教学型工业级模板收敛v5)
  - [3. 依赖与构建](#3-依赖与构建)
  - [4. 烧录与串口工具](#4-烧录与串口工具)
  - [5. 目录结构](#5-目录结构)
...
推荐先阅读 [`docs/INDEX.md`](./docs/INDEX.md)，再进入具体模块专题。
...
这一轮改造不再只堆新功能，而是把工程继续收敛到“教学型工业级模板”：

- **驱动接口统一**
  - `usart1`、`pwm`、`photoresistor`、`encoder`、`ssd1306` 统一向 `stm_status_t + config struct + try/blocking` 风格靠拢。
  - 旧接口保留为兼容包装，但不再作为新代码主入口。
- **板级逻辑设备入口**
  - 新增 `bsp/board_devices`，把默认控制台、显示、状态灯、编码器、环境光的实例绑定收拢到 BSP。
  - `app` 层只面向逻辑角色，不再直接依赖 `USART1` / `TIM2_CH1` / `ADC1_IN1` 等底层命名。
- **诊断基础设施**
  - 新增 `stm_log`、`stm_fault`、`stm_assert`，让 `DEBUG_LOG`、`ASSERT_LEVEL` 等构建选项真正被代码消费。
  - `main/app` 初始化失败路径不再散落 `while (1)`，而是统一走故障停机入口。
- **文档入口统一**
  - 新增 [`docs/INDEX.md`](./docs/INDEX.md) 作为文档导航。
  - 新增 [`docs/DRIVER_API_GUIDE.md`](./docs/DRIVER_API_GUIDE.md) 说明新的驱动接口约定。
